...
#读取路径
path = r"E:\learning\VSCode+python\python\hot deform beh\fitData\70%"
#保存路径
pathResult = r"E:\learning\VSCode+python\python\hot deform beh\Stress\70%"
   ...
   while j < df.shape[1]:
      ...
      for k in range(df.shape[0]):
         ...
         if m <= maxStrain and abs(m-df.iloc[k,j]) <= precision:
            ...
            #将应变点数据存储到arrData
            arrData[q][p] = m
            arrData[q][p+1] = df.iloc[k,j]
            arrData[q][p+2]  = df.iloc[k,j+1]
            #维系arrData的行
            q = q +1
            #维系应变量
            m = m+step
