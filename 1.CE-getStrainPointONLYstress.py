...
for file in dirs:
   ...
   while j < df.shape[1]:
      ...
      j = j+cols
      print("j={}".format(j))
      #将一个温度(一个列单元)的全部应变点数据放入数组
      #print(arrData)
      result = pd.concat([result,pd.DataFrame(np.array(arrData))],axis=1)
   result.to_excel(pathResult+r"\rate"+file,index=False)
